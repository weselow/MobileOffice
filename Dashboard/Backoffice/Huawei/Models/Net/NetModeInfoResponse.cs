...
namespace Dashboard.Backoffice.Huawei.Models.Net
{
    ...
    public class NetModeInfoResponse
    {
        [XmlElement(ElementName = "NetworkMode")]
        public string NetworkMode { get; set; } = string.Empty;
        [XmlElement(ElementName = "NetworkBand")]
        public string NetworkBand { get; set; } = string.Empty;
        [XmlElement(ElementName = "LTEBand")]
...
/*
     Режим работы сети (1) - информация (мне кажется, что речь идет о настройке, на каких частотах модем может подключаться. Если у кого-то есть лучшее имя, не стесняйтесь писать Язык)
